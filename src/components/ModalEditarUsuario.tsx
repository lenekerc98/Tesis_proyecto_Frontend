...
interface Usuario {
    ...
    rol?: string; // Agregado para soportar el rol
}
...
export const ModalEditarUsuario: React.FC<ModalProps> = ({ isOpen, onClose, user, onSave }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Obtener valor del rol (Truco: usamos getElementById porque no creamos ref específica arriba para no romper mucho código)
        const roleSelect = document.getElementById("roleSelector") as HTMLSelectElement;
        const roleValue = roleSelect ? parseInt(roleSelect.value) : null;

        // 2. Construimos el objeto base (Nombre, Email, Activo y ROL siempre van)
        ...
        const datosParaEnviar: any = {
            ...
            role_id: roleValue // Enviamos el rol seleccionado
        };
        ...
        >
            ...
            >
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-4">
                        <label className="form-label fw-bold small text-muted">Rol del Usuario</label>
                        <select
                            className="form-select"
                            defaultValue={user.rol === "admin" ? "0" : "1"}
                            ref={(el) => {
                                // Guardamos referencia al select para leerlo en submit
                                if (el) (el as any).roleValue = el;
                            }}
                            id="roleSelector"
                        >
